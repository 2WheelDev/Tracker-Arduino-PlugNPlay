...
void recoverDatabase() {
  ...
  // Rutas// 1) Extraer el nombre base (sin “.db”)
char baseName[80];
strncpy(baseName, logFileName, sizeof(baseName));
...
snprintf(newPath, sizeof(newPath),
         "%s%s-corrupta.db",// /db/ + miLog-corrupta.db
         DB_DIR, baseName);

// 3) Renombrar
if (SD.rename(oldPath, newPath)) {
  Serial.println("Renombrado OK");
} else {
...
bool writeBufferToSQLite(const GPSData* buffer, int count, const char* name) {
  ...
  const char* sql =
    ...
    "timestamp,lat,lon,ele,speed,course,sat,hdop)"
    " VALUES (?,?,?,?,?,?,?,?);";
...
bool guardarResumenRuta() {
  ...
  const char* sql =
    ...
    "(timestamp_inicio, timestamp_fin,tiempo_mov,tiempo_parado, duracion_segundos, "
  ...
  sqlite3_bind_text  (stmt, 2, ts_fin,             -1, SQLITE_TRANSIENT);
    sqlite3_bind_double(stmt, 3, (float)tiempoMovimientoMs/1000.0);
        sqlite3_bind_double(stmt, 4, (float)tiempoParadoMs/1000.0);


  sqlite3_bind_int   (stmt, 5, (int)duracion);
  sqlite3_bind_double(stmt, 6, distanciaTotalKm);
  sqlite3_bind_int   (stmt, 7, (int)totalMuestrasGPS);
  sqlite3_bind_double(stmt, 8, hzMedia);
